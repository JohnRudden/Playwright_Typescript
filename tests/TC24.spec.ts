...
test("Test Case 24: Download Invoice after purchase order " , {tag: []}, async ({browserName, page, navbarPage, modals, productsPage, cartPage, registerUser, checkoutPage, deleteAccountPage}) => {
 ...
 await test.step("Review order " , async ()=> {
  ...
  await test.step.skip("Click download invoice button and verify invoice has been downloaded" , async () => {
    if (browserName !== 'webkit') {
    const downloadPromise = page.waitForEvent('download');
    await page.getByText('Download Invoice').click();
    const download = await downloadPromise;
    expect(download.url()).toContain('/download_invoice/');
    expect(download.suggestedFilename()).toContain('invoice.txt');
    }
  });

  await test.step("Click continue..." , async () => {
    await page.getByText("Continue").click();
    expect(page.url()).not.toContain('payment_done')
  })
